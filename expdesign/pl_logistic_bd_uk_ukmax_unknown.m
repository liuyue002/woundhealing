...
T=25;
t=linspace(0,T,nt);
% let's say ukmax is unknown to the experimenter
% but uk0 in [0,1] is known
ukmax=1200;
params=[0.45,0.15,1,3900, ukmax];
...
clean_data=sol_richards_control_ukmaxunknown(t,params,C0,u_d,u_K);
sigma=20;
...
figtitle=sprintf('pl_richards_bd_uk_ukmaxunknown_%s_params=[%g,%g,%g,%g,%g],sigma=%g,tau0=%g,tau=%g,uK0=%g,rng=%g',string(datetime,'yyyyMMdd_HHmmss'),params,sigma,tau0,tau,u_K0,seed);
%% MLE
fixed=[0,0,1,0,0];
fixed_param_val=params;
numeric_params={T,nt,u_d,u_K};
lb_opt=[0,0,0,0,0];
ub_opt=[5,5,9,50000,50000];
opt.lb=lb_opt;
opt.ub=ub_opt;
opt.logging=true;
opt.alg=1;
[mle,mle_sigma,max_l] = optimize_likelihood_general(@richards_ctrl_sq_err_ukmaxunknown,fixed_param_val,fixed,noisy_data,numeric_params,C0,opt);
optimal_param_vals=fixed_param_val';
optimal_param_vals(fixed==0)=mle;

mle_soln=sol_richards_control_ukmaxunknown(t,optimal_param_vals,C0,u_d,u_K);
...
lb=[ 0.10, 0.00, 7,   2000,  200];
ub=[ 0.50, 0.40, 9,   3000, 5000];
opt.ub=[10,10,10,99000, 99000];
opt.alg=1;
param_names={'r','\delta','\gamma','K','ukmax'};
...
for param=1:num_params
    ...
    for i=mle_idx+1:numpts+1
        ...
        [minimizer,~,max_ls(param,i)] = optimize_likelihood_general(@richards_ctrl_sq_err_ukmaxunknown,initial,fixed_params,noisy_data,numeric_params,C0,opt);
