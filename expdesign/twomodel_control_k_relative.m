%% set up
f = @(C,r,d,gamma,K,uk) r*C.*(1-(C./(K*(1-uk))).^gamma)-d*C;
dfdC= @(C,r,d,gamma,K,uk) r*(1-(1+gamma)*(C./(K*(1-uk))).^gamma)-d;
...
alpha=30000; % weight of control cost
omega=0.1; % control update rate
uklim = 0.5; %upper bound for uk
bangbang=false;
