%% set up
f = @(C,r,d,gamma,K,ur) r*(1+ur)*C.*(1-(C./K).^gamma)-d*C;
dfdC= @(C,r,d,gamma,K,ur) r*(1+ur)*(1-(1+gamma)*(C./K).^gamma)-d;
...
alpha=7e5; % weight of control cost
omega=0.1; % control update rate
urlb = 0; % lower bound for ur
urub = 0.5; % upper bound for ur
bangbang=false;
...
filename=sprintf('simulations/twomodel_control_r_relative_%s_alpha=%.2f,omega=%.2f',string(datetime,'yyyyMMdd_HHmmss'),alpha,omega);
makeplot=true;
...
if makeplot
    ...
    hold off
    xlabel('t');
    legend('C_1','C_2','|C_1-C_2|','u*1000+1000');
    xlim([0,T]);
    ylim([0,6000]);
...
for iter=1:1000
    ...
    if makeplot
        ...
        uplot.XData=t;
        uplot.YData=arrayfun(ur,t)*1000+1000;
    ...
    for i=1:upts
        ...
        if bangbang
            ...
            % urnumnew(i)=sol.ur;
            urnumnew(i)=fmincon(objective, 0.1, [],[],[],[],urlb,urub,[],fminconopts);
        end
    end
