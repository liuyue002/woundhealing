...
% };
%%
load(['simulations/',dim1_files{1}]);
param_names={'D_0','r','\alpha','\beta','\gamma','\eta','K'};
figure(figg);
param_inds=[1,2,7];
nexttile(tilecoordinate(1,1));
...
nexttile(tilecoordinate(1,5));set(gca,'visible','off');
%%
load(['simulations/',dim1_files{2}]);
param_names={'D_0','r','\alpha','\beta','\gamma','\eta','K'};
figure(figg);
param_inds=[1,2,7,6];
nexttile(tilecoordinate(2,1));
...
%%
load(['simulations/',dim1_files{3}]);
param_names={'D_0','r','\alpha','\beta','\gamma','\eta','K'};
figure(figg);
param_inds=[1,2,7,5];
nexttile(tilecoordinate(3,1));
...
%%
load(['simulations/',dim1_files{4}]);
param_names={'D_0','r','\alpha','\beta','\gamma','\eta','K'};
figure(figg);
param_inds=[1,2,7,3,4];
nexttile(tilecoordinate(4,1));
...
%% now plot the 2D ones

load(['simulations/',dim2_files{1}]);
param_inds=[1,2,7];
figure(figg);
for i=1:3
    param=param_inds(i);
    ...
    xx=param_vals(param,:);
    yy=max_ls(param,:)-max(max_ls(param,:));
    plot(xx,yy,'c:','DisplayName',param_names{param});
end
%%
load(['simulations/',dim2_files{2}]);
param_inds=[1,2,7,6];
figure(figg);
for i=1:4
    param=param_inds(i);
...
for i=1:4
    ...
    nexttile(tilecoordinate(4,5));
    hold on
    plot(xx,yy,'c:','DisplayName','\alpha (Fig. 2)');
    i=5;

    param=param_inds(i);
    xx=param_vals(param,:);
    yy=max_ls(param,:)-max(max_ls(param,:));
    plot(xx,yy,'r:','DisplayName','\beta (Fig. 2)');
...
saveas(figg,sprintf('figure/xy%d_1d2dcompare_figtable.fig',experiment));
saveas(figg,sprintf('figure/xy%d_1d2dcompare_figtable.png',experiment));
saveas(figg,sprintf('figure/xy%d_1d2dcompare_figtable.eps',experiment),'epsc');
...
function tilenum=tilecoordinate(x,y)
tilenum=(x-1)*5+y;
end