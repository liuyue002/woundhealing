...
load('/home/liuy1/Documents/woundhealing/simulations/woundhealing_1d_20211115_143713_D0=500,r=0.05,alpha=1,beta=1,gamma=1,n=0.mat');
...
[overall_minimizer,sigma2,max_l,param_str,grad,hessian] = optimize_likelihood(fixed,fixed_param_val,lb,ub,noisy_data,T,t_skip,x_skip);

fprintf(['Overall max likelihood param is: ',repmat('%.3f,',size(overall_minimizer)),'sigma2=%.3f,\n'],overall_minimizer,sigma2);
...
% dXdtheta=zeros(N,num_free_params);
% [~,model_data0,~] = woundhealing_1d(optimal_param_vals,T,0);
% model_data0=model_data0(1:t_skip:end,1:x_skip:end);
% model_data0=reshape(model_data0,N,1);
% free_param_count=0;
% for param=1:num_params
%     if fixed(param)
%         continue;
...
%     dtheta=0.0001;
%     param_vals = optimal_param_vals;
%     param_vals(param)=param_vals(param)+dtheta;
%     [~,model_data1,~] = woundhealing_1d(param_vals,T,0);
%     model_data1=model_data1(1:t_skip:end,1:x_skip:end);
%     model_data1=reshape(model_data1,N,1);
%     dXdtheta(:,param) = (model_data1-model_data0)./dtheta;
% end
