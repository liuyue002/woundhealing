%% set up
f = @(C,r,d,gamma,K,ud) r*C.*(1-(C./K).^gamma)-(d+ud)*C;
dfdC= @(C,r,d,gamma,K,ud) r*(1-(1+gamma)*(C./K).^gamma)-(d+ud);
...
filename=sprintf('simulations/twomodel_control_ud_%s_alpha=%.2f,omega=%.2f',string(datetime,'yyyyMMdd_HHmmss'),alpha,omega);
makeplot=true;
...
for iter=1:1000
    ...
    if makeplot
        ...
        udnewplot.YData=udnumnew*10000;
        drawnow;
