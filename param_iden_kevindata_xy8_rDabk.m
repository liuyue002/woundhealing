...
fixed_param_val=[3184,0.024,1.234,0.135,1,0,2217];
lb=[3000, 0.015, 1.1, 0.08, 0.60, 0.00, 2200];
ub=[3350, 0.030, 1.3, 0.18, 1.60, 1.00, 2230];
...
lb_opt=[ 100, 0.01, 0.1, 0.01, 0.1, 0,  500]; %[0,0,0,0,0,0,0]
ub_opt=[5000, 1.00, 2.0, 9.00, 9.0, 4, 5000]; %[20000,5,10,10,10,10,10000]

figtitle=sprintf(['fixed=[',repmat('%d,',size(fixed)),'],fixedparamval=[',repmat('%g,',size(fixed)),'],kevindata,threshold=%g,tskip=%d,xskip=%d',',4_cmaes'],fixed,fixed_param_val,threshold,t_skip,x_skip);
...
%% overall minimizer
[overall_minimizer,sigma,max_l,param_str,~,~] = optimize_likelihood(fixed,fixed_param_val,lb_opt,ub_opt,noisy_data,numeric_params,t_skip,x_skip,threshold,ic,1,NaN,NaN,scaling);
