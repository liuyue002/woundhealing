...
load('simulations/kevindata_triangle_xy7_20220405_raw.mat');
...
fixed_param_val=[10061,0.164,1,1,1,1.044,2626];
lb=[ 9800, 0.162, 0.8, 0.8, 0.60, 1.02, 2620];
ub=[10400, 0.166, 1.3, 1.3, 1.60, 1.07, 2634];
...
ub_opt=[50000, 1.00, 9.0, 9.0, 9.0, 4, 5000]; %[20000,5,10,10,10,10,10000]

figtitle=sprintf(['fixed=[',repmat('%d,',size(fixed)),'],fixedparamval=[',repmat('%g,',size(fixed)),'],kevindata,threshold=%g,tskip=%d,xskip=%d',',7'],fixed,fixed_param_val,threshold,t_skip,x_skip);
